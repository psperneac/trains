...
interface CopyPlacesModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCopy: (sourceGameId: string, overwrite: boolean) => Promise<void>;
}

export default function CopyPlacesModal({ isOpen, onClose, onCopy }: CopyPlacesModalProps) {
      ...
      >
        <div className="mt-3 text-center">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Select a source game to copy all its places to the current game.
            Places with duplicate names will be skipped unless overwrite is enabled.
          </p>
          <select
          ...
          <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-100 mb-4">
            ...
            <label htmlFor="overwrite" className="text-sm font-medium text-gray-700 cursor-pointer select-none">
              Overwrite existing places with same name
            </label>
          </div>

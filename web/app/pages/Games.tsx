...
export default function Games() {
  ...
  const handleDeleteGame = (game: GameDto) => {
  ...
  const handleDeleteGameConfirm = async () => {
    ...
    setIsSubmitting(true);
    try {
      await useGameStore.getState().deleteGame(gameToDelete.id);
      setShowDeleteGameModal(false);
      setGameToDelete(null);
    } catch (err: any) {
      setModalError(err.message || 'Failed to delete game');
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleLockGame = async (game: GameDto) => {
    try {
      await useGameStore.getState().lockGame(game.id);
    } catch (err: any) {
      setModalError(err.message || 'Failed to lock game');
    }
  };

  const handleUnlockGame = async (game: GameDto) => {
    try {
      await useGameStore.getState().unlockGame(game.id);
    } catch (err: any) {
      setModalError(err.message || 'Failed to unlock game');
    }
  };
                  ...
                  .map((game) => {
                    ...
                    return (
                      <tr key={game.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {game.locked ? (
                            <span className="inline-flex px-2 py-1 text-xs font-medium rounded-md bg-orange-100 text-orange-700 border border-orange-200">
                              Locked
                            </span>
                          ) : (
                            <span className="inline-flex px-2 py-1 text-xs font-medium rounded-md bg-green-100 text-green-700 border border-green-200">
                              Open
                            </span>
                          )}
                        </td>
          ...
          >
            <div className="mt-3 text-center">
              ...
              <p className="text-gray-600 mb-2">
                Are you sure you want to delete game <strong>{gameToDelete.name}</strong>?
              </p>
              <p className="text-gray-500 text-sm mb-6">
                This will also delete all players, their places and vehicles, place connections, and all places associated with this game. This action cannot be undone.
              </p>
              {modalError && (
                <div className="mb-4 bg-red-100 border border-red-400 text-red-700 px-3 py-2 rounded text-sm">
                  {modalError}

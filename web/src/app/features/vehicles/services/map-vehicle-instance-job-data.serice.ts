...
import {
  MapVehicleInstanceJobActions,
  MapVehicleInstanceJobSelectors
} from '../store/map-vehicle-instance-job.store';
...
export class MapVehicleInstanceJobDataService {
  ...
  mapVehicleInstanceJobsByPlayerAndMapId$ = (playerId: string, mapId: string) =>
    this.store.pipe(select(MapVehicleInstanceJobSelectors.ByPlayerAndMap(playerId, mapId)));
  mapVehicleInstanceJobsByVehicleId$ = (vehicleId: string) =>
    this.store.pipe(select(MapVehicleInstanceJobSelectors.ByVehicle(vehicleId)));
  ...
  createMapVehicleInstanceJob() {
    this.store.dispatch(
      MapVehicleInstanceJobActions.selectOne({
        payload: {
          name: '',
          description: '',
...
export const mapVehicleInstanceJobsResolveFn = () =>
  inject(MapVehicleInstanceJobDataService).resolveMapVehicleInstanceJobs();
export const createMapVehicleInstanceJobGuardFn = () =>
  inject(MapVehicleInstanceJobDataService).createMapVehicleInstanceJob();
